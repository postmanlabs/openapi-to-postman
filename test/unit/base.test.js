...
describe('CONVERT FUNCTION TESTS ', function() {
  ...
  describe('The convert Function', function() {
    var pathPrefix = VALID_OPENAPI_PATH + '/test.json',
      ...
      pathPrefix7 = VALID_OPENAPI_PATH + '/readOnly.json',
    ...
    it('Should respects readOnly and writeOnly properties in requestBody or response schema' +
     specPath7, function(done) {
      ...
      Converter.convert({ type: 'string', data: openapi }, { schemaFaker: true }, (err, conversionResult) => {
        let requestBody = conversionResult.output[0].data.item[0].item[0].item[0].request.body.raw,
          responseBody = conversionResult.output[0].data.item[0].item[0].item[0].response[0].body;
        expect(err).to.be.null;
