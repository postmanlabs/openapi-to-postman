...
describe('CONVERT FUNCTION TESTS ', function() {
  ...
  describe('The convert Function', function() {
    ...
    describe('create folders according to their tags ' + tagsFolderSpec, function() {
      ...
      it('should create the folder when tag is only given in the operation and not in the root level', function(done) {
        Converter.convert({ type: 'file', data: tagsFolderSpec },
          { folderStrategy: 'tag', schemaFaker: true }, (err, conversionResult) => {
            expect(conversionResult.output[0].data.item[2].name).to.equal('pets');
            expect(conversionResult.output[0].data.item[2].item[0].request.name).to.equal('List all pets');
            expect(conversionResult.output[0].data.item[2].item[1].request.name).to.equal('Create pets');
            done();
          });
      });

      it('should create the folder when tag is defined at root level and also used in the operation', function(done) {
        Converter.convert({ type: 'file', data: tagsFolderSpec },
          { folderStrategy: 'tag', schemaFaker: true }, (err, conversionResult) => {
            expect(conversionResult.output[0].data.item[1].name).to.equal('single pet');
            expect(conversionResult.output[0].data.item[1].item[0].request.name).to.equal('Info for a specific pet');
            done();
          });
      });
    });
    describe('for invalid requestNameSource option', function() {
      ...
      it('for invalid request name, converter should throw an error', function(done) {
        ...
        Converter.convert({ type: 'string', data: openapi }, { requestNameSource: 'uKnown' },
          (err, conversionResult) => {
            expect(err).to.be.null;
