...
function getPropName (propNameType) {
  return propNameType.slice(0, propNameType.indexOf(','));
}
...
function createMinifiedSchema (schema, key, nestingLevel, allSchemas) {
  ...
  if (schema.type === 'object') {
    ...
    _.forEach(_.get(schema, 'properties'), (propSchema, propName) => {
      ...
      names.push(propName);
      if ((type === 'object' || type === 'array') && nestingLevel > 0) {
    ...
    if ((arrayType === 'object' || arrayType === 'array') && nestingLevel > 0) {
 ...
 * @returns {*} null
 */
function resolveSchema (schemaKey, allSchemas) {
  ...
  if (minifiedSchema.array && !_.includes(validTypes, minifiedSchema.array)) {
    resolveSchema(minifiedSchema.array, allSchemas);
...
function extractCommonComponents (schemas, components, options) {
  ...
  _.forEach(commonProps, (commonProp, schemaKeys) => {
    ...
    if (commonProp.length >= 3) {
      firstSchemaKey = schemaKeys.slice(0, schemaKeys.indexOf(','));
      ...
      _.set(components, 'schemas.' + componentName, getCommonComponent(commonProp, schemaKeys, allSchemas));
  ...
  _.forEach(schemas, (schema, schemaKey) => {
    resolveSchema(schemaKey, allSchemas);
