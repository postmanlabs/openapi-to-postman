...
module.exports = {
   ...
   */
  dereferenceRoot: function(spec) {
    let dereferencedSpec = Object.assign({}, spec);
    // seenRef = [] TODO:Add circular references handling here
    const that = this;
    traverseUtility(dereferencedSpec).forEach(function (property) {
      if (
        ...
        !property.$ref.includes('components/schemas')
      ) {
        this.update(that.dereferenceFromOutOfComponents(dereferencedSpec, property.$ref));
      }
    });
