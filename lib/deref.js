...
module.exports = {
   ...
   */
  resolveAllOf: function (schemaArr, stackLimit, parameterSourceOption, components, stack = 0) {
    ...
    if (schemaArr.length === 1) {
      ...
      return this.resolveRefs(schemaArr[0], stackLimit, parameterSourceOption, components, stack);
    }

    // generate one object for each schema
    let indivObjects = schemaArr.map((schema) => {
        return this.resolveRefs(schema, stackLimit, parameterSourceOption, components, stack);
      }).filter((schema) => {
   ...
   */
  resolveRefs: function (schema, stackLimit, parameterSourceOption, components, stack = 0) {
    ...
    if (schema.anyOf) {
      return this.resolveRefs(schema.anyOf[0], stackLimit, parameterSourceOption, components, stack);
    }
    if (schema.oneOf) {
      return this.resolveRefs(schema.oneOf[0], stackLimit, parameterSourceOption, components, stack);
    }
    if (schema.allOf) {
      return this.resolveAllOf(schema.allOf, stackLimit, parameterSourceOption, components, stack);
    }
    if (schema.$ref) {
      ...
      if (resolvedSchema) {
        return this.resolveRefs(resolvedSchema, stackLimit, parameterSourceOption, components, stack);
      }
    ...
    if (schema.type === 'object' || schema.hasOwnProperty('properties')) {
        ...
        for (prop in schema.properties) {
          if (schema.properties.hasOwnProperty(prop)) {
            ...
            tempSchema.properties[prop] = this.resolveRefs(property,
              stackLimit, parameterSourceOption, components, stack);
          }
        }
        return tempSchema;
      }
      ...
      tempSchema.items = this.resolveRefs(schema.items, stackLimit, parameterSourceOption, components, stack);
      return tempSchema;
    }
