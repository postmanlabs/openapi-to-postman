  ...
  resolveRefs: function (schema, parameterSourceOption, components, schemaResolutionCache,
    ...
    if (schema.type === 'object' || schema.hasOwnProperty('properties')) {
      ...
      /*
        ...
        FOR CONVERSION - set both properties as minimum and valid as possible for schema faking (to lessen faked items)
          We have enforced limit to maxItems as 100, set by Json schema faker option
      */
      if (resolveFor === 'CONVERSION') {
        // Override maxItems to default (2) if no minItems present
        (!_.has(schema, 'minItems') && _.has(schema, 'maxItems') && schema.maxItems >= 2) && (schema.minItems = 2);

        // Override maxItems to minItems if minItems is available
        (_.has(schema, 'minItems') && schema.minItems >= 2) && (schema.maxItems = schema.minItems);
