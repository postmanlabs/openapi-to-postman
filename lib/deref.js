const _ = require('lodash'),
  ...
  RESOLVE_REF_DEFAULTS = {
    ...
    stackLimit: 10,
    isAllOf: false,
    includeOptionalChanges: false
  },
  DEFAULT_SCHEMA_UTILS = require('./30XUtils/schemaUtils30X'),
  traverseUtility = require('traverse'),
  PROPERTIES_TO_ASSIGN_ON_CASCADE = ['type', 'nullable', 'required'];
...
module.exports = {
  ...
  resolveAllOf: function (schema, parameterSourceOption, components, {
    ...
    analytics = {},
    includeOptionalChanges = RESOLVE_REF_DEFAULTS.includeOptionalChanges
  }) {
      ...
      if (_.has(schema, 'properties') || _.has(schema, 'additionalProperties')) {
        ...
        for (prop in schema.properties) {
          if (schema.properties.hasOwnProperty(prop)) {
            ...
            // Mark all properties as required if optional properties are also to be suggested if missing
            if (includeOptionalChanges) {
              if (_.isArray(tempSchema.required)) {
                tempSchema.required.push(prop);
              }
              else {
                tempSchema.required = [prop];
              }
            }
        ...
        !_.isEmpty(tempSchema.required) && (tempSchema.required = _.uniq(tempSchema.required));
        return tempSchema;
      }
