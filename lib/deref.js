...
module.exports = {
  ...
  resolveRefs: function (schema, bodyType, components, stack = 0) {
    ...
    if (schema.anyOf) {
      return this.resolveRefs(schema.anyOf[0], bodyType, components, stack);
    }
    if (schema.oneOf) {
      return this.resolveRefs(schema.oneOf[0], bodyType, components, stack);
    }
    if (schema.allOf) {
      return this.resolveAllOf(schema.allOf, bodyType, components, stack);
    }
    if (schema.$ref) {
    ...
    if (schema.type === 'object' || schema.hasOwnProperty('properties')) {
      ...
      if (schema.hasOwnProperty('properties')) {
        ...
        for (prop in schema.properties) {
          if (schema.properties.hasOwnProperty(prop)) {
            ...
            tempSchema.properties[prop] = this.resolveRefs(tempSchema.properties[prop], bodyType, components, stack);
          }
        }
        return tempSchema;
      }
      ...
      schema.maxItems = 2;
      schema.minItems = 2;
      schema.items = this.resolveRefs(schema.items, bodyType, components, stack);
    }
