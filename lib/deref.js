      ...
      if (schema.hasOwnProperty('type')) {
        ...
        if (resolveFor === 'CONVERSION' && resolveTo === 'schema') {
          if (!schema.hasOwnProperty('format')) {
            schema.__default__ = '<' + schema.type + '>';
          }
          else if (type.hasOwnProperty(schema.type)) {
            schema.__default__ = type[schema.type][schema.format];

            // in case the format is a custom format (email, hostname etc.)
            // https://swagger.io/docs/specification/data-models/data-types/#string
            if (!schema.__default__ && schema.format) {
              schema.__default__ = '<' + schema.format + '>';
            }
          }
          else {
            schema.__default__ = '<' + schema.type + (schema.format ? ('-' + schema.format) : '') + '>';
          }
        }
      }
