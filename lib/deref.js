...
module.exports = {
   ...
   */
  resolveAllOf: function (schemaArr, stackSize, parameterSourceOption, components, stack = 0) {
    ...
    if (schemaArr.length === 1) {
      ...
      return this.resolveRefs(schemaArr[0], stackSize, parameterSourceOption, components, stack);
    }

    // generate one object for each schema
    let indivObjects = schemaArr.map((schema) => {
        return this.resolveRefs(schema, stackSize, parameterSourceOption, components, stack);
      }).filter((schema) => {
   ...
   */
  resolveRefs: function (schema, stackSize, parameterSourceOption, components, stack = 0) {
    ...
    if (schema.anyOf) {
      return this.resolveRefs(schema.anyOf[0], stackSize, parameterSourceOption, components, stack);
    }
    if (schema.oneOf) {
      return this.resolveRefs(schema.oneOf[0], stackSize, parameterSourceOption, components, stack);
    }
    if (schema.allOf) {
      return this.resolveAllOf(schema.allOf, stackSize, parameterSourceOption, components, stack);
    }
    if (schema.$ref) {
      ...
      if (resolvedSchema) {
        return this.resolveRefs(resolvedSchema, stackSize, parameterSourceOption, components, stack);
      }
    ...
    if (schema.type === 'object' || schema.hasOwnProperty('properties')) {
        ...
        for (prop in schema.properties) {
          if (schema.properties.hasOwnProperty(prop)) {
            ...
            tempSchema.properties[prop] = this.resolveRefs(property,
              stackSize, parameterSourceOption, components, stack);
          }
        }
        return tempSchema;
      }
      ...
      tempSchema.items = this.resolveRefs(schema.items, stackSize, parameterSourceOption, components, stack);
      return tempSchema;
    }
