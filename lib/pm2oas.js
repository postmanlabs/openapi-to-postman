  ...
  return _.map(reqGroups, (reqGroup, reqGroupPath) => {
    let operation = {},
      ...
      getItemProp = (itemTree, prop) => {
        return _.findLast(itemTree, (itemInfo, index) => {
    ...
    if (itemTreeInfo) {
      ...
      if (!_.isEmpty(tag)) {
        operation.tag = {
          name: tag,
          description: getItemProp(itemTreeInfo, 'description')
        };
      }
    }
...
function flattenPmCollection (collection, flattenCollection, folderInfoArr, options) {
  ...
  _.forEach(collection.item, (item) => {
    ...
    if (_.isArray(item.item)) {
      flattenPmCollection(item, flattenCollection, folderInfoArr, options);
    }
    else if (!_.isEmpty(item.request)) {
      flattenCollection.push(_.assign({}, item, { itemTree: folderInfoArr }));
    }
  });
}
