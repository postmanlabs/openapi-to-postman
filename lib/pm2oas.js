  ...
  _.forEach(pathVars, (pathVar) => {
    ...
    _.isEmpty(allPathVars[pathVar.index]) &&
      (allPathVars[pathVar.index] = {
        key: pathVar.key,
        description: getDescription(pathVar.description),
        values: [],
        examples: []
      });
...
function mergeQueries (queries, options) {
  ...
  _.forEach(queries, (query) => {
    ...
    _.isEmpty(allQueries[query.key]) &&
      (allQueries[query.key] = { description: getDescription(query.description), values: [], examples: [] });
  ...
  _.forEach(headers, (header) => {
    ...
    if (!_.includes(EXCLUDED_HEADERS, _.toLower(_.get(header, 'key')))) {
      _.isEmpty(allHeaders[header.key]) &&
        (allHeaders[header.key] = { description: getDescription(header.description), values: [], examples: [] });
...
function mergePmRequests (requests, options) {
  ...
  return _.map(reqGroups, (reqGroup, reqGroupPath) => {
    ...
    // set first non-empty request name and description as operation summary and description
    _.forEach(reqGroup, (req) => {
      let reqName = _.get(req, 'request.name'),
        reqDesc = _.get(req, 'request.request.description');

      if (!_.isEmpty(reqName)) { operation.summary = reqName; }

      if (!_.isEmpty(reqDesc)) { operation.description = reqDesc; }

      if (operation.summary && operation.description) { return false; }
    });
    ...
    if (itemTreeInfo) {
      ...
      if (!_.isEmpty(tag)) {
        let tagDesc = getDescription(getItemProp(itemTreeInfo, 'description'));
...
async function generateOAS (requests, options) {
  ...
  oasSpec.info.description = getDescription(_.get(requests, '[0].itemTree[0].description')) || DEFAULT_SPEC_DESCRIPTION;
  ...
  if (!_.isEmpty(globalSecurityObj)) {
    ...
    _.set(oasSpec.components, 'securitySchemes.' + securitySchemeName, globalSecurityObj);
    securitySchemeCount += 1;
  }

  // collect all schemas from requests and response bodies and extract common components
  _.forEach(mergedRequests, (mergedRequest) => {
    ...
    if (!_.isEmpty(mergedRequest.requestBodies)) {
      schemaName = utils.generateSchemaName(_.camelCase(
        (_.isEmpty(mergedRequest.summary) ? `${method} ${path}` : mergedRequest.summary),), allSchemas);
      _.forEach(_.get(mergedRequest.requestBodies, 'content'), (contentObj, contentType) => {

        if (_.includes(JSON_CONTENT_TYPES, contentType)) {
          contentObj.schemaName = schemaName;
          allSchemas[schemaName] = {
            path: `["${path}"].${method}.requestBodies.content.${contentType}.schema`,
    ...
    if (!_.isEmpty(mergedRequest.responses)) {
      _.forEach(mergedRequest.responses, (response, responseCode) => {
        let responseSchema = _.get(response, 'content.application/json.schema');

        if (responseSchema) {
          schemaName = utils.generateSchemaName(_.camelCase((_.isEmpty(mergedRequest.summary) ? `${method} ${path}` :
            mergedRequest.summary) + responseCode + ' Response'), allSchemas);

          response.content['application/json'].schemaName = schemaName;
          allSchemas[schemaName] = {
            path: `["${path}"].${method}.responses.${responseCode}.content.application/json.schema`,
  ...
  // extract common components from all schemas
  extractedSchemas = extractCommonComponents(_.mapValues(allSchemas, 'schema'), oasSpec.components, options);
  ...
  _.forEach(mergedRequests, (mergedRequest) => {
    ...
    _.assign(operation, _.pick(mergedRequest, ['summary', 'description', 'servers']));

    !_.isEmpty(allParameters) && (operation.parameters = allParameters);

    if (!_.isEmpty(mergedRequest.requestBodies)) {
      _.forEach(_.get(mergedRequest.requestBodies, 'content'), (contentObj, contentType) => {
        if (contentObj.schemaName) {
          _.set(mergedRequest, 'requestBodies.' + contentType + '.schema', extractedSchemas[contentObj.schemaName]);
          _.unset(contentObj, 'schemaName');
        }
      });
    ...
    if (!_.isEmpty(mergedRequest.responses)) {
      _.forEach(mergedRequest.responses, (response, responseCode) => {
        let schemaName = _.get(response, 'content.application/json.schemaName');

        if (schemaName) {
          _.set(mergedRequest.responses, responseCode + '.content.application/json.schema',
            extractedSchemas[schemaName]);
          _.unset(response, 'content.application/json.schemaName');
        }
      });
  ...
  if (options.outputFormat === 'yaml') {
    try {
      return jsYaml.dump(oasSpec, {
        skipInvalid: true, // don't throw on invalid types and skip pairs and single values with such types.
        noRefs: true // don't convert duplicate objects into references
      });
    }
    catch (e) {
      throw OpenApiErr('Error while converting to YAML');
    }
  }
  return oasSpec;
}
