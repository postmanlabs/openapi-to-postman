...
module.exports = {
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    _.forEach(reqParams.query, (queryParam) => {
      this.convertToPmQueryParameters(queryParam, REQUEST_TYPE.ROOT, components, options, schemaCache)
        .forEach((pmParam) => {
          // Modify the request query params for the test suites
          pmParam = this.overwriteRequestQueryParamByTests(pmParam, operation, operationItem, options);
    ...
    // Modify the request paths variables for the test suites
    reqParams.path = this.overwriteRequestPathByTests(reqParams.path, operation, operationItem, options);
    ...
    _.forEach(reqParams.header, (header) => {
      // Modify the request header for the test suites
      header = this.overwriteRequestHeaderByTests(header, operation, operationItem, options);
      ...
      _.forOwn(swagResponse.content, (content, contentType) => {
        if (contentType) {
          ...
          if (contentType === APP_JSON) {
            let resolvedSchema, assignPmVariablesCounter = 0;
            ...
            if (testSuite.assignPmVariables) {
              ...
              let assignPmVariablesForOperation = testSuiteAssignPmVariables.find((or) => {
              ...
              if (assignPmVariablesForOperation && assignPmVariablesForOperation.environmentVariables) {
                ...
                assignEnvironmentVariables.forEach((environmentVariable) => {
                  if (environmentVariable.responseProp) {
                    // Set variable name
                    let varName = operation.operationId + '.' + environmentVariable.responseProp;
                    ...
                    requestsTests.push(
                      ...
                      'jsonData.' + environmentVariable.responseProp + ');\n' +
                      '};\n');
                    console.log('- pm.environment for "' + operation.operationId + '" - use {{' + varName + '}} ' +
                      'as variable for "reponse.' + environmentVariable.responseProp + '"');
                    assignPmVariablesCounter++;
                  }
                });
              }
  ...
  overwriteRequestPathByTests: function (requestPathVariables, operation, operationItem, options) {
    options = _.merge({}, defaultOptions, options);
    let testRequestPathVariables = Object.assign([], requestPathVariables), // clone requestPathVariables
      testSuite = {},
      overwriteValues = [],
      testSuiteOverwriteRequest = {},
      testSuiteOverwriteRequests = [];
