...
module.exports = {
  ...
  addCollectionItemsUsingTags: function(spec, generatedStore, components, options, schemaCache, sourceMap) {
    ...
    _.forEachRight(tagFolders, (tagFolder, tagName) => {
      ...
      _.forEach(tagFolder.requests, (request) => {
        itemGroup.items.add(this.convertRequestToItem(spec, request, components, options, schemaCache, variableStore, sourceMap));
      });
  ...
  convertChildToItemGroup: function (openapi, child, components, options, schemaCache, variableStore, sourceMap) {
    ...
    if (resource.requestCount > 1) {
      ...
      if (resource.childCount === 1 && options.collapseFolders) {
        ...
        return this.convertChildToItemGroup(openapi, resourceSubChild, components, options, schemaCache, variableStore, sourceMap);
      }
      /* eslint-enable */
      ...
      for (i = 0, requestCount = resource.requests.length; i < requestCount; i++) {
        itemGroup.items.add(
          this.convertRequestToItem(openapi, resource.requests[i], components, options, schemaCache, variableStore, sourceMap)
        );
      }
    ...
    if (resource.requests.length === 1) {
      return this.convertRequestToItem(openapi, resource.requests[0], components, options, schemaCache, variableStore, sourceMap);
    }
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
   ...
   */
  convertToPmResponse: function(response, code, originalRequest, components, options, schemaCache, operationItem, sourceMap) {
    options = _.merge({}, defaultOptions, options);
    var responseHeaders = [],
    ...
    const sourceMapKey = `#/paths/${encodeURIComponent(`/${operationItem.path}`)}/${operationItem.method}/responses/${code}`;
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore, sourceMap) {
    ...
    const sourceMapRequestKey = `#/paths/${encodeURIComponent(`/${operationItem.path}`)}/${operationItem.method}.request`;
