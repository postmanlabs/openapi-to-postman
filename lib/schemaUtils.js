...
module.exports = {
  ...
  */
  ...
  convertToPmBody: function(requestBody, requestType, components, options) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.convertToPmBodyData(contentObj[URLENCODED], requestType, URLENCODED, PARAMETER_SOURCE.REQUEST, '', components, options);
      ...
      rDataMode = 'formdata';
      bodyData = this.convertToPmBodyData(contentObj[FORM_DATA], requestType, FORM_DATA, PARAMETER_SOURCE.REQUEST, '', components, options);
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options) {
    ...
    _.forEach(reqParams.header, (header) => {
      item.request.addHeader(this.convertToPmHeader(header, REQUEST_TYPE.ROOT, PARAMETER_SOURCE.REQUEST, components, options));
    });
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        thisOriginalRequest.url += this.convertToPmQueryArray(reqParams, REQUEST_TYPE.EXAMPLE, components, options).join('&');
        // setting headers
        _.forEach(reqParams.header, (header) => {
          originalRequestHeaders.push(this.convertToPmHeader(header, REQUEST_TYPE.EXAMPLE, PARAMETER_SOURCE.REQUEST, components, options));
        });
        thisOriginalRequest.header = originalRequestHeaders;
        // setting request body
        try {
          exampleRequestBody = this.convertToPmBody(operationItem.properties.requestBody, REQUEST_TYPE.EXAMPLE, components, options);
  ...
  */
    ...
    _.forOwn(schemaPathItems, (pathItemObject, path) => {
      ...
      for (i = 0; i < schemaPathLength; i++) {
        if (
          ...
          // this segment is not a match. try the next pathItemObject of the _.forOwn
          return true;
        }

        if (schemaPathSegments[i].startsWith('{')) {
          // the schema says this is a path variable
          pathItemValues.push({
            key: schemaPathSegments[i].substring(1, schemaPathSegments[i].length - 1),
            value: pathToMatchSegments[i]
          });
        }
      }
      ...
      if (pathItemObject[method.toLowerCase()]) {
        ...
        // aggregate local + global parameters
  ...
  checkValueAgainstSchema: function (property, jsonPathPrefix, value, schemaPathPrefix, schema, callback) {
   ...
   * @param {*} transactionPathPrefix the jsonpath-prefix for this validation (will be prepended to all identified mismatches)
   * @param {*} schemaPath the applicable pathItem defined at the schema level
   * @param {*} schemaPathPrefix the jsonpath-prefix for this validation rule (will be prepended to all identified mismatches)
  ...
  checkPathVariables: function (
    ...
    async.map(determinedPathVariables, (pathVar, cb) => {
      ...
      this.checkValueAgainstSchema(mismatchProperty, transactionPathPrefix + '[' + pathVar.key + ']',
        pathVar.value,
        schemaPathVar.pathPrefix + schemaPathVar.name, schemaPathVar.schema, cb);
    }, (err, res) => {
      if (err) {
        callback(err);
      }
      ...
      _.each(schemaPathVariables, (pathVar) => {
        if (!_.find(determinedPathVariables, (param) => { return param.key === pathVar.name; })) {
          mismatches.push({
            ...
            reason: `The requried path variable ${pathVar.name} was not found in the transaction`
          });
        }
      });
  ...
  checkRequestHeaders: function (headers, transactionPathPrefix, schemaPath, callback) {
  ...
  checkResponseHeaders: function (code, headers, transactionPathPrefix, schemaPathPrefix, schemaPath, callback) {
    // 0. Need to find relevant response from schemaPath.responses
    let schemaResponse = schemaPath.responses[code],
      responsePathPrefix = code,
      schemaHeaders,
      schemaHeader,
    ...
    if (!schemaResponse) {
      schemaResponse = schemaPath.responses.default;
  ...
  checkRequestBody: function (requestBody, transactionPathPrefix, schemaPathPrefix, schemaPath, callback) {
    ...
    let jsonSchemaBody = _.get(schemaPath, ['requestBody', 'content', 'application/json', 'schema']),
      mismatches = [];
    if (requestBody.mode === 'raw' && jsonSchemaBody) {
      // only raw for now
  ...
  checkResponseBody: function (code, headers, transactionPathPrefix, schemaPathPrefix, schemaPath, callback) {
    // 0. Need to find relevant response from schemaPath.responses
    // TODO:
    let schemaResponse = _.get(schemaPath, ['responses', code, 'content', 'application/json']),
      responsePathPrefix = code,
      schemaHeaders,
      schemaHeader,
      mismatchProperty = 'RESPONSE_HEADER',
    ...
    if (!schemaResponse) {
      schemaResponse = _.get(schemaPath, ['responses', 'default', 'content', 'application/json']);
      responsePathPrefix = 'default';
    }
