...
const { formatDataPath, checkIsCorrectType, isKnownType } = require('./common/schemaUtilsCommon.js'),
  ...
  { getRemoteReferencesArray } = require('./remoteRefSolver'),
...
module.exports = {
  ...
  */
  ...
  mapProcessRelatedFiles(rootFiles, inputData, origin, version, format, toBundle = false, options = {}) {
    let bundleFormat = format,
      parsedRootFiles = rootFiles.map((rootFile) => {
        let parsedContent;
        if (rootFile.parsed) {
          parsedContent = rootFile.parsed;
        }
        else {
          parsedContent = parseFileOrThrow(rootFile.content);
        }
   ...
   * @returns {undefined} - nothing
   */
  async resolveRemoteRefsMultiFile(input, options) {
    let remoteRefRes =
    await getRemoteReferencesArray(input.rootFiles, options.origin, options.remoteRefsResolver),
      foundRemoteRefs = remoteRefRes.map((remoteResult) => {
        return remoteResult.remoteRefs;
      }).flat();
    input.data.push(...foundRemoteRefs);
  },
