...
* @param {boolean} includeDeprecatedProperties - Whether to include the deprecated properties
 * @returns {undefined} undefined
 */
function verifyDeprecatedProperties(resolvedSchema, includeDeprecatedProperties) {
  traverseUtility(resolvedSchema.properties).forEach(function (property) {
    if (property) {
      if (property.deprecated === true && includeDeprecatedProperties === false) {
        this.delete();
      }
    }
  });
...
function safeSchemaFaker(oldSchema, resolveTo, resolveFor, parameterSourceOption, components,
  schemaFormat, indentCharacter, schemaCache, stackLimit, includeDeprecatedProperties) {
  ...
  if (resolvedSchema.properties) {
    ...
    verifyDeprecatedProperties(resolvedSchema, includeDeprecatedProperties);
  }
...
function shouldAddDeprecatedOperation (tempRequest, options) {
  return !tempRequest.properties.deprecated ||
    (tempRequest.properties.deprecated === true && options.includeDeprecatedProperties === true);
}
      ...
      _.forEach(commonPathVars, (variable) => {
        ...
        fakedData = options.schemaFaker ?
          safeSchemaFaker(variable.schema || {}, options.requestParametersResolution, PROCESSING_TYPE.CONVERSION,
            ...
            options.stackLimit, options.includeDeprecatedProperties) : '';
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  getParametersForPathItem: function(localParams, options) {
    let tempParam,
      ...
      includeDeprecatedProperties = options.includeDeprecatedProperties !== undefined ?
        options.includeDeprecatedProperties : true;

    _.forEach(localParams, (param) => {
      tempParam = param;
      let shouldAddDeprecated = (includeDeprecatedProperties ||
        includeDeprecatedProperties === false && tempParam.deprecated !== true);
    ...
    if (bodyObj.example && (resolveTo === 'example' || !bodyObj.schema)) {
      ...
      if (options.schemaFaker) {
        ...
        bodyData = safeSchemaFaker(bodyObj.schema || {}, resolveTo, PROCESSING_TYPE.CONVERSION, parameterSourceOption,
          ...
          options.includeDeprecatedProperties);
      }
      else {
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components,
    ...
    var reqName,
      ...
      // itemParams = operationItem.properties.deprecated,
      // options.includeDeprecatedProperties
