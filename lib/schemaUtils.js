  ...
  */
  ...
  checkValueAgainstSchema: function (property, jsonPathPrefix, value, schemaPathPrefix, schema,
    ...
    if (schema && schema.type) {
      if (!schemaTypeToJsValidator[schema.type]((needJsonMatching ? jsonValue : value))) {
        ...
        if (!options.shortValidationErrors) {
          ...
          reason += ', found invalid JSON instead';
        }
        else if (Array.isArray(jsonValue)) {
          reason += ', found an array instead';
        }
        else {
          reason += `, found a ${typeof value} instead`;
        }
      ...
      if (schema.type === 'array' || schema.type === 'object') {
        try {
          ...
          res = validate((needJsonMatching ? jsonValue : value));
        }
        catch (e) {
