 ...
 */
var varVal;  // for example variables
...
module.exports = {
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  getExampleData: function(exampleObj, components, options) {
    ...
    var example,
      exampleKey, exampleKey1, example2;

    if (typeof exampleObj !== 'object') {
      return '';
    }
    // console.log("{ 'Example-1': " + JSON.stringify((exampleObj)["Example-1"]) + " }");

    exampleKey = Object.keys(exampleObj)[0];
    exampleKey1 = Object.keys(exampleObj)[1];

    if(typeof Object.keys(exampleObj)[1] === "string") {
      example2 = exampleObj[exampleKey1];
      // return example value if present else example is returned

      if (example2.hasOwnProperty('$ref')) {
        example2 = this.getRefObject(example2.$ref, components, options);
      }
      if (example2.hasOwnProperty('value')) {
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmBody: function(requestBody, requestType, components, options, schemaCache) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      else if (contentObj.hasOwnProperty(TEXT_PLAIN)) { bodyType = APP_JSON; }
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmResponse: function(response, code, originalRequest, components, options, schemaCache) {
    ...
    if (responseBodyWrapper.contentTypeHeader) {
      ...
      responseHeaders.push({ key: 'Content-Type', value: 'application/json' }); // TEXT_PLAIN
    ...
    sdkResponse.postmanpreviewlanguage = previewLanguage;
    arr = sdkResponse;
    return sdkResponse;
  },
  ...
  //   var stringVal = JSON.stringify(str);
  //   var slashVal = stringVal.split("/");
  //   return JSON.stringify(slashVal);
  // },

  // replaceKeyWithId: function(a, b){
  //   const matches1 = this.findCollectionVariablesFromPath(a);
  //   const replaceWith = a.replace(/{{[a-zA-z]*}}|\\/g, this.makeid(7)).replace(/}}/g, '');
  //   return replaceWith;
  ...
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        // setting URL
        // displayforEx = this.replaceKeyWithId(displayforEx, varVal);
