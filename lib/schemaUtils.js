...
module.exports = {
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  /**
   * Generates name for schema based on name, suffix is added if same name exists
   *
   * @param {String} initialName initial name
   * @param {Object} schemas all existing schemas
   * @returns {Strings} schema name
   */
  generateSchemaName: function (initialName, schemas) {
    let suffix = 1,
      schemaName = initialName;

    // check whether name already exist
    while (schemas[schemaName]) {
      schemaName = initialName + suffix;
      suffix += 1;
    }
    return schemaName;
  },
