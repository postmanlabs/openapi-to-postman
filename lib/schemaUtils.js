    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        if(options.separateRequests) {
          for(i = 0; i < options.separateRequests.length; i++) {
            if(options.separateRequests[i].url == reqUrl && options.separateRequests[i].method == item.request.method) {
              separate = true;
              break;
            }
          }
        ...
        if(separate) {
          convertedResponses.forEach((convertedResponse) => {
            let tempItem = item;
            tempItem.responses.add(convertedResponse);
            items.push(tempItem);
          });
        }
        else {
