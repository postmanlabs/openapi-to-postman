...
function safeSchemaFaker (oldSchema, resolveTo, resolveFor, parameterSourceOption, components,
  ...
  resolvedSchema = deref.resolveRefs(oldSchema, parameterSourceOption, components, {
    ...
    includeOptionalChanges: options.includeOptionalChanges
  });
...
module.exports = {
  ...
  */
  ...
  getExampleValues: function (examples, example) {
    let allExamples = [];

    if (_.isArray(examples)) {
      allExamples = _.map(examples, (example) => {
   ...
   * @param {Array} examples - Examples mentioned for corresponding OpenAPI object (params, request/response body etc)
   * @param {String} parameterSourceOption tells that the schema object is of request or response
  ...
  checkValueAgainstSchema: function (property, jsonPathPrefix, txnParamName, value, schemaPathPrefix, openApiSchemaObj,
    examples, parameterSourceOption, components, options, schemaCache, jsonSchemaDialect, callback) {
      ...
      async.map(compositeSchema, (elementSchema, cb) => {
        setTimeout(() => {
          this.checkValueAgainstSchema(property, jsonPathPrefix, txnParamName, value,
            ...
            elementSchema, examples, parameterSourceOption, components, options, schemaCache, jsonSchemaDialect, cb);
        }, 0);
      }, (err, results) => {
        let sortedResults;
    ...
    // When processing a reference, schema.type could also be undefined
      ...
      if (isKnownType(schema)) {
        ...
        if (options.includeExampleChanges) {
          !_.isUndefined(schema.example) && (examples.push(schema.example));

          let isExampleMatching = (examples, value) => {
            return _.some(examples, (example) => {
              /**
               * As header, params, path variable values are stored always as string,
               * for such properties parse the string and than evaluate if they match or not
               */
              if (_.includes(['QUERYPARAM', 'PATHVARIABLE', 'HEADER', 'RESPONSE_VARIABLE'], property) &&
                typeof value === 'string') {
                try {
                  return JSON.parse(value) === example;
                }
                catch (e) {
                ...
                return _.isEqual(value, example);
              }
            });
          };

          if (!_.isEmpty(examples) && !isExampleMatching(examples, valueToUse)) {
            let exampleToBeAdded = _.head(examples),
              isExampleValid = _.isEmpty(validateSchema(schema, exampleToBeAdded, options, jsonSchemaDialect)),
              mismatchObj;
            ...
            if (isExampleValid) {
              mismatchObj = {
                ...
                transactionJsonPath: jsonPathPrefix,
                schemaJsonPath: schemaPathPrefix,
                reasonCode: 'INCORRECT_EXAMPLE',
                reason: `A ${humanPropName} contains incorrect example according to definition.`
              };
              ...
              if (options.suggestAvailableFixes) {
                mismatchObj.suggestedFix = {
                  key: txnParamName,
                  actualValue: valueToUse,
                  suggestedValue: exampleToBeAdded
                };
              }
    ...
    async.map(determinedPathVariables, (pathVar, cb) => {
      ...
      setTimeout(() => {
        ...
        this.checkValueAgainstSchema(mismatchProperty,
          ...
          this.getExampleValues(schemaPathVar.examples, schemaPathVar.example),
  ...
  checkMetadata(transaction, transactionPathPrefix, schemaPath, pathRoute, options, callback) {
    ...
    if (!options.validateMetadata && !options.includeDescriptiveChanges) {
      return callback(null, []);
    }
  ...
  checkQueryParams(queryParams, requestUrl, transactionPathPrefix, schemaPath, components, options,
    ...
    return async.map(requestQueryParams, (pQuery, cb) => {
      ...
      setTimeout(() => {
        ...
        this.checkValueAgainstSchema(mismatchProperty,
          ...
          schemaParam.schema,
          this.getExampleValues(schemaParam.examples, schemaParam.example),
          PARAMETER_SOURCE.REQUEST,
          components, options, schemaCache, jsonSchemaDialect, (err, mismatches) => {
            if (err) {
              return cb(err);
            }

            if (options.includeDescriptiveChanges) {
              let paramDescription = this.getParameterDescription(schemaParam);

              if (pQuery.description !== paramDescription) {
                let mismatchObj = {
                  property: mismatchProperty,
                  transactionJsonPath: transactionPathPrefix + `[${index}].description`,
                  schemaJsonPath: schemaParam.pathPrefix + '[?(@.name==\'' + schemaParam.name + '\')]',
                  reasonCode: 'INCORRECT_VALUE',
                  reason: 'The query parameter contains incorrect description'
                };

                if (options.suggestAvailableFixes) {
                  mismatchObj.suggestedFix = {
                    key: 'description',
                    actualValue: pQuery.description,
                    suggestedValue: paramDescription
                  };
                }
                mismatches.push(mismatchObj);
              }
      ...
      let mismatches = [],
        mismatchObj,
        filteredQueryParams = _.filter(resolvedSchemaParams, (q) => {
          return options.includeOptionalChanges || q.required;
        });

      _.each(filteredQueryParams, (qp) => {
        if (!_.find(requestQueryParams, (param) => {
          ...
          mismatchObj = {
            ...
            reason: `The ${q.required ? 'required ' : ''}query parameter "${qp.name}" ` +
    ...
    return async.map(reqHeaders, (pHeader, cb) => {
      ...
      setTimeout(() => {
        ...
        this.checkValueAgainstSchema(mismatchProperty,
          ...
          schemaHeader.schema,
          this.getExampleValues(schemaHeader.examples, schemaHeader.example),
          PARAMETER_SOURCE.REQUEST,
          components, options, schemaCache, jsonSchemaDialect, (err, mismatches) => {
            if (err) {
              return cb(err);
            ...
            if (options.includeDescriptiveChanges) {
              let headerDescription = this.getParameterDescription(schemaHeader);

              if (pHeader.description !== headerDescription) {
                let mismatchObj = {
                  property: mismatchProperty,
                  transactionJsonPath: transactionPathPrefix + `[${index}].description`,
                  schemaJsonPath: schemaHeader.pathPrefix + '[?(@.name==\'' + schemaHeader.name + '\')]',
                  reasonCode: 'INCORRECT_VALUE',
                  reason: 'The header contains incorrect description'
                };

                if (options.suggestAvailableFixes) {
                  mismatchObj.suggestedFix = {
                    key: 'description',
                    actualValue: pHeader.description,
                    suggestedValue: headerDescription
                  };
                }
                mismatches.push(mismatchObj);
              }
      ...
      _.each(_.filter(schemaHeaders, (h) => {
        ...
        return (options.includeOptionalChanges || h.required) && !_.includes(IMPLICIT_HEADERS, _.toLower(h.name));
      }), (header) => {
        if (!_.find(reqHeaders, (param) => { return param.key === header.name; })) {
          ...
          mismatchObj = {
            ...
            reasonCode: 'MISSING_IN_REQUEST',
            reason: `The ${q.required ? 'required ' : ''}header "${header.name}" ` +
              'was not found in the transaction'
          };
  ...
  checkResponseHeaders: function (schemaResponse, headers, transactionPathPrefix, schemaPathPrefix,
    ...
    return async.map(resHeaders, (pHeader, cb) => {
            ...
            if (options.includeDescriptiveChanges) {
              ...
              if (pHeader.description !== headerDescription) {
                let mismatchObj = {
                  property: mismatchProperty,
                  transactionJsonPath: transactionPathPrefix + `[${index}].description`,
                  schemaJsonPath: schemaPathPrefix + '.headers[' + pHeader.key + ']',
                  reasonCode: 'INCORRECT_VALUE',
                  reason: 'The header contains incorrect description'
                };
      ...
      _.each(_.filter(schemaHeaders, (h, hName) => {
        ...
        h.name = hName;
        // exclude non-required and implicit header from further validation
        return (options.includeOptionalChanges || h.required) && !_.includes(IMPLICIT_HEADERS, _.toLower(hName));
      }), (header) => {
        if (!_.find(resHeaders, (param) => { return param.key === header.name; })) {
          ...
          mismatchObj = {
            ...
            reasonCode: 'MISSING_IN_REQUEST',
            reason: `The ${q.required ? 'required ' : ''}response header "${header.name}" ` +
              'was not found in the transaction'
          };
  ...
  checkRequestBody: function (requestBody, transactionPathPrefix, schemaPathPrefix, schemaPath,
    ...
    bodyExample = _.get(schemaPath, ['requestBody', 'content', jsonContentType, 'example']);
    bodyExamples = _.get(schemaPath, ['requestBody', 'content', jsonContentType, 'examples']);
    ...
    if (requestBody && requestBody.mode === 'raw' && jsonSchemaBody) {
      setTimeout(() => {
        return this.checkValueAgainstSchema(mismatchProperty,
          ...
          jsonSchemaBody,
          this.getExampleValues(bodyExamples, bodyExample),
          PARAMETER_SOURCE.REQUEST,
          components,
      ...
      _.forEach(_.get(urlencodedBodySchema, 'properties'), (propSchema, propName) => {
        let resolvedProp = {
            ...
            example: _.get(propSchema, 'example')
          },
      ...
      return async.map(requestBody.urlencoded, (uParam, cb) => {
        ...
        _.each(resolvedSchemaParams, (uParam) => {
          ...
          if (!_.find(requestBody.urlencoded, (param) => { return param.key === uParam.name; }) &&
            (options.includeOptionalChanges || uParam.required)) {
  ...
  checkResponseBody: function (schemaResponse, body, transactionPathPrefix, schemaPathPrefix,
    ...
    bodyExample = _.get(schemaResponse, ['content', jsonContentType, 'example']);
    bodyExamples = _.get(schemaResponse, ['content', jsonContentType, 'examples']);
