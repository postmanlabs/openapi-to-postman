...
module.exports = {
  ...
  */
  ...
  convertToPmCollectionVariables: function(serverVariables, keyName, serverUrl = '') {
    ...
    if (serverVariables) {
      _.forOwn(serverVariables, (value, key) => {
        let description = (value.required ? '(Requried) ' : '') + (value.description || '') +
          (value.enum ? ' (is one of ' + value.enum + ')' : '');
  ...
  convertPathVariables: function(type, providedPathVars, commonPathVars, components, options) {
    ...
    if (type === 'root' || type === 'method') {
      ...
      _.forEach(commonPathVars, (variable) => {
        let description = (variable.required ? '(Requried) ' : '') + (variable.description || '') +
          (variable.enum ? ' (is one of ' + variable.enum + ')' : '');
        variables.push({
          key: variable.name,
  ...
  convertToPmQueryParameters: function(param, requestType, components, options) {
    ...
    let description = (param.required ? '(Requried) ' : '') + (param.description || '') +
          (param.enum ? ' (is one of ' + param.enum + ')' : '');
  ...
  convertParamsWithStyle: function(param, paramValue) {
    var paramType = param.schema.type,
      paramNameArray,
      pmParams = [],
      description = (param.required ? '(Requried) ' : '') + (param.description || '') +
        (param.enum ? ' (is one of ' + param.enum + ')' : ''),
      // converts: {a: [1,2,3]} to:
  ...
  convertToPmHeader: function(header, requestType, parameterSource, components, options) {
    ...
    reqHeader.description = (header.required ? '(Requried) ' : '') + (header.description || '') +
      (header.enum ? ' (is one of ' + header.enum + ')' : '');
