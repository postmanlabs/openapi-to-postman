...
module.exports = {
  ...
  */
   ...
   */
  addCollectionItemsUsingTags: async function(spec, generatedStore, components, options, schemaCache) {
    ...
    paths = Object.keys(paths).map((key) => {
      return { ...paths[key], 'path': key };
    });
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      // discard the leading slash, if it exists
      if (paths[i].path[0] === '/') {
        paths[i].path = paths[i].path.substring(1);
      }

      // extracting common parameters for all the methods in the current path item
      if (paths[i].hasOwnProperty('parameters')) {
        commonParams = paths[i].parameters;
      }

      // storing common path/collection vars from the server object at the path item level
      if (paths[i].hasOwnProperty('servers')) {
        pathLevelServers = paths[i].servers;
        ...
        collectionVariables = this.convertToPmCollectionVariables(
          ...
          this.fixPathVariableName(paths[i].path), // the name of the variable
    ...
    tagFolders = Object.keys(tagFolders).map((key) => {
      return { ...tagFolders[key], 'tagName': key };
    });
    tagFolders.reverse();
    for (let i = 0; i < tagFolders.length; i++) {
      // eslint-disable-next-line one-var
      var itemGroup = new sdk.ItemGroup({
        name: tagFolders[i].tagName,
        description: tagFolders[i].description
      });

      for (let j = 0; j < tagFolders[i].requests.length; j++) {
        itemGroup.items.add(await this.convertRequestToItem(
          spec, tagFolders[i].requests[j], components, options, schemaCache, variableStore
        ));
      }
  ...
  convertRequestToItem: async function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    if (operation.responses) {
      ...
      responses = Object.keys(operation.responses).map((key) => {
        return { ...operation.responses[key], 'code': key };
      });
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        swagResponse = responses[i];
        if (responses[i].$ref) {
          swagResponse = this.getRefObject(responses[i].$ref, components, options);
        }
  ...
  */
  ...
  getMissingSchemaEndpoints: async function (schema, matchedEndpoints, components, options, schemaCache) {
    ...
    schemaPaths = Object.keys(schemaPaths).map((key) => {
      return { ...schemaPaths[key], 'schemaPath': key };
    });

    for (let i = 0; i < schemaPaths.length; i++) {
      pathKeys = Object.keys(schemaPaths[i]);
      for (let j = 0; j < pathKeys.length; j++) {
        schemaJsonPath = `$.paths[${schemaPaths[i].schemaPath}].${_.toLower(pathKeys[j])}`;
        if (METHODS.includes(pathKeys[j]) && !matchedEndpoints.includes(schemaJsonPath)) {
          let mismatchObj = {
            property: 'ENDPOINT',
            transactionJsonPath: null,
            schemaJsonPath,
            reasonCode: 'MISSING_ENDPOINT',
            reason: `The endpoint "${_.toUpper(pathKeys[j])} ${schemaPaths[i].schemaPath}" is missing in collection`,
            endpoint: _.toUpper(pathKeys[j]) + ' ' + schemaPaths[i].schemaPath
          };

          if (options.suggestAvailableFixes) {
            let operationItem = _.get(schemaPaths[i], pathKeys[j], {}),
              convertedRequest,
              variables = rootCollectionVariables,
              path = schemaPaths[i].schemaPath,
              request;

            // add common parameters of path level
            operationItem.parameters = this.getRequestParams(operationItem.parameters,
              _.get(schemaPaths[i], 'parameters'), components, options);
            ...
            // storing common path/collection vars from the server object at the path item level
            if (!_.isEmpty(_.get(schemaPaths[i], 'servers'))) {
              let pathLevelServers = schemaPaths[i].servers;
            ...
            request = {
              ...
              method: pathKeys[j],
              // eslint-disable-next-line max-len
              path: schemaPaths[i].schemaPath[0] === '/' ? schemaPaths[i].schemaPath.substring(1) : schemaPaths[i].schemaPath,
              properties: operationItem,
              type: 'item',
              servers: _.isEmpty(_.get(schemaPaths[i], 'servers'))
            };

            // convert request to collection item and store collection variables
            // eslint-disable-next-line max-len
            convertedRequest = await this.convertRequestToItem(schema, request, components, options, schemaCache, variables);
