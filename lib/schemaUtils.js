  ...
  addCollectionItemsUsingTags: function(spec, generatedStore, components, options, schemaCache) {
    ...
    _.forEach(paths, (currentPathObject, path) => {
      ...
      _.forEach(pathMethods, (pathMethod) => {
        ...
        if (_.isEmpty(localTags)) {
          ...
          if (!tempRequest.properties.deprecated ||
    ...
    _.forEachRight(tagFolders, (tagFolder, tagName) => {
      ...
      _.forEach(tagFolder.requests, (request) => {
        if (!request.properties.deprecated ||
          (request.properties.deprecated === true && options.includeDeprecatedProperties === true)) {
          itemGroup.items.add(
  ...
  convertChildToItemGroup: function (openapi, child, components, options,
    ...
    if (resource.requestCount > 1) {
      ...
      for (i = 0, requestCount = resource.requests.length; i < requestCount; i++) {
        ...
        if (!resource.requests[i].properties.deprecated ||
          (resource.requests[i].properties.deprecated === true && options.includeDeprecatedProperties === true)) {
          itemGroup.items.add(
    ...
    if (resource.requests.length === 1) {
      if (!resource.requests[0].properties.deprecated ||
        (resource.requests[0].properties.deprecated === true && options.includeDeprecatedProperties === true)) {
