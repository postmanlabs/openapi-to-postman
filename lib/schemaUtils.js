...
module.exports = {
  ...
  */
   ...
   */
  async mapProcessRelatedFiles(rootFiles, inputData, origin, version, format, toBundle = false, options = {}, remoteRefResolver) {
    let bundleFormat = format,
      ...
      data = toBundle ?
        await this.getBundledFileData(parsedRootFiles, inputData, origin, bundleFormat, version, options, remoteRefResolver) :
    ...
    if (inputRelatedFiles.rootFiles && inputRelatedFiles.rootFiles.length > 0) {
      try {
        res.output.data = await this.mapProcessRelatedFiles(inputRelatedFiles.rootFiles, inputRelatedFiles.data,
          inputRelatedFiles.origin, version, inputRelatedFiles.bundleFormat, toBundle, options, inputRelatedFiles.remoteRefResolver);
