        ...
        if (_.isObject(paramValue)) {
          ...
          _.forEach(extractedParams, (extractedParam) => {
            pmParams.push(this.generateSdkParam({
              ...
              value: _.get(extractedParam, 'value', ''),
              description,
              disabled
            }, _.get(param, 'in')));
          });
    ...
    if (requestBody && requestBody.mode === 'raw' && jsonSchemaBody) {
      ...
      _.forEach(_.get(urlencodedBodySchema, 'properties'), (propSchema, propName) => {
        ...
        if (isPropSeparable && propSchema.type === 'array' && pSerialisationInfo.explode) {
          ...
          if (!_.includes(['array', 'object'], _.get(propSchema, 'items.type'))) {
            ...
            resolvedSchemaParamsMap[propName] = _.assign({}, resolvedProp, {
              schema: _.get(propSchema, 'items'),
              isResolvedParam: true
            });
          }
          ...
          if (pSerialisationInfo.style === 'deepObject') {
            ...
            const childParams = this.extractChildParamSchema(propSchema,
              propName, { required: resolvedProp.required || false, description: resolvedProp.description });

            _.forEach(childParams, (childParam) => {
              resolvedSchemaParamsMap[childParam.name] = childParam;
            });
          }
          else {
            // add schema of all properties instead entire object
          ...
          resolvedSchemaParamsMap[propName] = resolvedProp;
        }
      });
      ...
      return async.map(requestBody.urlencoded, (uParam, cb) => {
        let mismatches = [],
          ...
          schemaParam = resolvedSchemaParamsMap[uParam.key];

        if (!schemaParam) {
          // skip validation of complex array params
          if (this.isParamComplexArray(uParam.key)) {
            let isCompositeSchema;

            const regex = /\[[\w]+\]/gm,
              newKey = uParam.key.replace(regex, '');

            schemaParam = resolvedSchemaParamsMap[newKey];
            isCompositeSchema = schemaParam && schemaParam.schema && (schemaParam.schema.anyOf ||
                schemaParam.schema.oneOf);

            if (isCompositeSchema) {
              const keys = uParam.key.match(regex),
                resolvedValue = _.reduce(keys, (accumulator, key, index) => {
                  const sanitizedKey = key.substring(1, key.length - 1);

                  accumulator[sanitizedKey] = accumulator[sanitizedKey] || {};

                  if (index === keys.length - 1) {
                    accumulator[sanitizedKey] = resolvedParamValue;
                  }
        ...
        _.forEach(_.flatten(res), (mismatchObj) => {
          if (!_.isEmpty(mismatchObj)) {
            let propertyName = getPropNameFromSchemPath(mismatchObj.schemaJsonPath),
              ...
              schemaParam = resolvedSchemaParamsMap[propertyName],
        ...
        _.each(_.values(resolvedSchemaParamsMap), (uParam) => {
