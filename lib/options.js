...
module.exports = {
  ...
  /**
   * Returns sync options for the syncCollection
   *
   * @param {string} [mode='document'] Describes use-case. 'document' will return an array
   * with all options being described. 'use' will return the default values of all options
   * @returns {mixed} An array or object (depending on mode) that describes available sync options
   */
  getSyncOptions: function(mode = 'document') {
    let syncOptionsArray = [
      {
        name: 'Sync response examples',
        id: 'syncExamples',
        type: 'boolean',
        default: false,
        description: 'Whether to sync response examples from the OpenAPI specification to the collection. ' +
    ...
    // Return options as default key-value pairs
    if (mode === 'use') {
      let defaultSyncOptions = {};

      syncOptionsArray.forEach((option) => {
        defaultSyncOptions[option.id] = option.default;
      });

      return defaultSyncOptions;
    }

    return syncOptionsArray;
  }
};
