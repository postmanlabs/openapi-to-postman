      ...
      if (defaultOptions.hasOwnProperty(id)) {
        ...
        switch (defaultOptions[id].type) {
          case 'boolean':
            ...
            break;
          case 'enum':
            const { availableOptions, allowOtherOptions, defaultValue } = defaultOptions[id];
            if (_.isString(userOptions[id]) &&
              _.map(availableOptions, _.toLower).includes(_.toLower(userOptions[id]))) {
              // ignore case-sensitivity for string options
              retVal[id] = _.toLower(retVal[id]);
            }
            else if (allowOtherOptions || availableOptions.includes(userOptions[id])) {
              retVal[id] = userOptions[id];
            }
            else {
              retVal[id] = _.isString(defaultValue) ? _.toLower(defaultValue) : defaultValue;
            }
