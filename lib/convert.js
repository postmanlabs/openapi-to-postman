...
var sdk = require('postman-collection'),
  ...
  converter = {
    ...
    convert: function (data, options, callback) {
      ...
      // setting it to the utils object so that we can use it in schemaFaker function
      if (typeof data === 'object') {
        dataSize = ((Buffer.byteLength(JSON.stringify(data), 'utf8') / 1024) / 1024); // size of input file in Mbs
      }
      else {
        dataSize = ((Buffer.byteLength(data, 'utf8') / 1024) / 1024); // size of input file in Mbs
      }
      util.openApiSpecSize = dataSize;
