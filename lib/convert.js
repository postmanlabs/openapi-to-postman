  ...
  converter = {
    ...
    generateCollection: function(specWrapper) {
      ...
      if (specWrapper.tags) {
        ...
        for (tag in tagsFolderObject.withTags) {
          if (tagsFolderObject.withTags.hasOwnProperty(tag)) {
            let itemGroup = util.convertTagToItemGroup(openapi, tag, tagsFolderObject.withTags[tag]);
            this.generatedStore.collection.items.add(itemGroup);
          }
        }
        let requestsWithoutTags = tagsFolderObject.withoutTags.requests;
