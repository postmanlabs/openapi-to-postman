...
var sdk = require('postman-collection'),
  ...
  converter = {
    ...
    /**
     * Adds items from the trie/tags as folders (or leaf requests) into the collection object
     * @param {object} specWrapper
     * @param {object} specWrapper.tree - the trie generated from util
     * @param {object} specwrapper.tags - the tag folder object generated from util
     * @returns {void}
     */
    generateCollection: function(specWrapper) {
      var tagsFolderObject, folderTree, openapi, child, tag;
      openapi = specWrapper.spec;
      if (specWrapper.tags) {
        // .tags will be truthy if the `folderStrategy` option is set to `tags`
        // the spec's requests will be grouped under their respective tags, instead of by folder
        // the .tree property will be empty
        tagsFolderObject = specWrapper.tags;
        for (tag in tagsFolderObject.tags) {
          if (tagsFolderObject.tags.hasOwnProperty(tag)) {
            let itemGroup = util.convertTagToItemGroup(openapi, tag, tagsFolderObject.tags[tag]);
            this.generatedStore.collection.items.add(itemGroup);
          }
        }
        let requestsWithoutTags = tagsFolderObject.requests;
        requestsWithoutTags.forEach((request) => {
          this.generatedStore.collection.items.add(util.convertRequestToItem(openapi, request));
        });
      }
      else {
        // this branch will be hit if the 'path' folderStrategy is used
        // we rely on the path-based tree to create folders, not the tags in the spec
        folderTree = specWrapper.tree;
    ...
    convert: function (data, options, callback) {
      ...
      util.options = {
        ...
        folderStrategy: (options.hasOwnProperty('folderStrategy') ? options.folderStrategy : 'path')
      };
      ...
      let specWrapper = {},
        folderObj, folderStrategy;
      specWrapper.spec = openapi;
      ...
      */
      ...
      if (folderStrategy === 'tag') {
        folderObj = util.generateFolderObjectFromTags(openapi);
        specWrapper.tags = folderObj;
      }
      else {
        folderObj = util.generateTrieFromPaths(openapi);
        specWrapper.tree = folderObj.tree;
      }
