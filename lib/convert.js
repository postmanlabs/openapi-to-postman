  ...
  converter = {
    ...
    convert: function (data, options, callback) {
      ...
      definedOptions.forEach((option) => {
        definedOptionsObject[option.id] = {
      ...
      // set default options
      for (let id in definedOptionsObject) {
        if (definedOptionsObject.hasOwnProperty(id)) {
          let option = definedOptionsObject[id];
          util.options[id] = (options.hasOwnProperty(id) && typeof options[id] === option.type ?
            options[id] : option.default);
        }
      }
