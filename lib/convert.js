  ...
  converter = {
    ...
    generateCollection: function(specWrapper) {
      ...
      if (specWrapper.tags) {
        ...
        for (tag in tagsFolderObject) {
          if (tagsFolderObject.hasOwnProperty(tag)) {
            if (tag === 'no-tag') {
              let requests = _.get(tagsFolderObject[tag], 'requests', []);
              ...
              let tagName = tag,
                itemGroup = util.convertTagToItemGroup(openapi, tagName, tagsFolderObject[tag]);
    ...
    convert: function (data, options, callback) {
      ...
      */
      folderStrategy = _.get(options, 'folderStrategy', 'path');
