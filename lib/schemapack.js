        ...
        return setTimeout(() => {
          ...
          return async.map(matchedPaths, (matchedPath, pathsCallback) => {
            const transactionPathVariables = _.get(transaction, 'request.url.variable', []),
              getLocalServersPathVars = () => {
                return matchedPath.path.servers.reduce((acc, current) => {
                  let newVarNames = current.hasOwnProperty('variables') ?
              ...
              localServersPathVars = matchedPath.path.hasOwnProperty('servers') ?
                getLocalServersPathVars() :
                [],
              isNotALocalServerPathVar = (pathVarName) => {
                return !localServersPathVars.includes(pathVarName);
              };
