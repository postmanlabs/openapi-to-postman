...
let path = require('path'),
  ...
  afterConvertProcess = (error, newOpenapi, schemaPackInstance, options, callback) => {
    let openapi,
      analysis,
      generatedStore = {},
      collectionJSON,
...
class SchemaPack {
  ...
  convert (callback) {
    ...
    if (options.resolveRemoteRefs) {
      getRemoteReferences(this.openapi, options.origin, options.remoteRefsResolver).then(() => {
        result = convertSwaggerToOpenapi(concreteUtils, this.openapi,
          (error, newOpenapi) => {
            afterConvertProcess(error, newOpenapi, this, options);
          });
      });
    }
    else {
      convertSwaggerToOpenapi(concreteUtils, this.openapi,
        (error, newOpenapi) => {
          afterConvertProcess(error, newOpenapi, this, options, callback);
        });
    }
  }
