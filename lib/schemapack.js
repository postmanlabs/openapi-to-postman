...
class SchemaPack {
  ...
  validate() {
    ...
    if (input.type === 'collection' || input.type === 'requests') {
      return this.validatePmInput();
    }
      ...
      this.validationResult = {
        result: false,
        reason: `Invalid input type (${input.type}). type must be one of file/json/string/collection/requests.`
      };
      return this.validationResult;
    }
    if (_.isEmpty(json)) {
  ...
  validatePmInput () {
    ...
    if (type === 'collection') {
      if (!_.isObject(data)) {
        this.validationResult = {
          result: false,
          reason: 'Input data for type (collection) should be an Object.'
        };
        return this.validationResult;
      }
      else if (!_.isString(_.get(data, 'info.name')) || !_.isArray(_.get(data, 'item'))) {
        this.validationResult = {
          result: false,
          reason: 'Input data for type (collection) should be valid collection.'
        };
        return this.validationResult;
      }
      ...
      if (!_.isArray(data)) {
        this.validationResult = {
          result: false,
          reason: 'Input data for type (requests) should be an Array.'
        };
        return this.validationResult;
      }

      _.forEach(data, (request, index) => {
        if (_.isEmpty(_.get(request, 'request'))) {
          invalidRequest = index;
          return false;
        }
      });
      ...
      if (invalidRequest) {
        this.validationResult = {
          result: false,
          reason: `Input data for type (requests) is invalid. Invalid request at index [${invalidRequest}]`
        };
        return this.validationResult;
      }
      this.requests = data;
  ...
  /**
   *
   * @description Converts Postman Collection v2 into OpenAPI spec
   *
   * @param {*} cb - return callback
   * @returns {*} - converted OpenAPI spec in desired format
   */
  async convertToSpec () {
    ...
    // flatten collection first
    if (collection) {
      pm2oas.flattenPmCollection(collection, requests, [], options);
    }

    try {
      convertedSpec = await pm2oas.generateOAS(requests, options);
    }
    catch (e) {
      return e;
    }
