...
const COLLECTION_NAME = 'Imported from OpenAPI 3.0',
  ...
  { getRemoteReferences } = require('./remoteRefSolver');

let path = require('path'),
  concreteUtils,
  pathBrowserify = require('path-browserify'),

  convertProcess = (error, newOpenapi, schemaPackInstance, options, callback) => {
    let openapi,
      analysis,
      generatedStore = {},
      collectionJSON,
      ...
      authHelper,
      schemaCache = {
        schemaResolutionCache: schemaPackInstance.schemaResolutionCache,
        schemaFakerCache: schemaPackInstance.schemaFakerCache
      };

    if (error) {
      return callback(error);
    }
    schemaPackInstance.openapi = newOpenapi;
    // this cannot be attempted before validation
    specComponentsAndUtils = { concreteUtils };
    Object.assign(specComponentsAndUtils, concreteUtils.getRequiredData(schemaPackInstance.openapi));

    // create and sanitize basic spec
    openapi = schemaPackInstance.openapi;
  ...
  validateTransacionProcess = (transactions, openapi, schemaPackInstance, options, callback) => {
    let schema = openapi,
      componentsAndPaths,
      analysis,
      schemaCache = {
    ...
    schemaPackInstance.openapi = openapi;
    // Only change the stack limit if the optimizeConversion option is true
    if (options.optimizeConversion) {
      // Deciding stack limit based on size of the schema, number of refs and number of paths.
    ...
    if (!schemaPackInstance.validated) {
...
class SchemaPack {
  ...
  validate() {
    ...
    this.inputFormat = specParseResult.inputFormat;
    this.validated = true;
    this.validationResult = {
      result: true
    };
  ...
  convert(callback) {
    ...
    if (options.resolveRemoteRefs) {
      remoteRefsInput = {
        fileName: '',
        content: '',
        parsed: { oasObject: this.openapi, inputFormat: this.inputFormat }
      };
      getRemoteReferences(remoteRefsInput, options.origin, options.remoteRefsResolver)
        .then(({ remoteRefs, specRoot }) => {
          let parsed = { oasObject: specRoot.parsed, inputFormat: this.inputFormat },
            res = schemaUtils.processRelatedFiles({
              type: 'folder',
              specificationVersion: this.openapi.openapi,
              rootFiles: [
                {
                  fileName: 'root.yaml',
          ...
          this.bundled = true;
          convertToOAS30IfSwagger(concreteUtils, res.output.data[0].bundledContent,
            (error, newOpenapi) => {
              convertProcess(error, newOpenapi, this, options, callback);
            });

        });
    }
    else {
      convertToOAS30IfSwagger(concreteUtils, this.openapi,
        (error, newOpenapi) => {
          convertProcess(error, newOpenapi, this, options, callback);
        });
    }
  ...
  validateTransaction(transactions, callback) {
    ...
    if (options.resolveRemoteRefs && !this.bundled) {
      remoteRefsInput = {
        fileName: '',
        content: '',
        parsed: { oasObject: this.openapi, inputFormat: this.inputFormat }
      ...
      getRemoteReferences(remoteRefsInput, options.origin, options.remoteRefsResolver)
        .then(({ remoteRefs, specRoot }) => {
          ...
          validateTransacionProcess(transactions, res.output.data[0].bundledContent, this, options, callback);
        });
    }
    else {
      validateTransacionProcess(transactions, this.openapi, this, options, callback);
    }
  }
  ...
  async bundle() {
    ...
    if (!input.rootFiles || input.rootFiles.length === 0) {
      ...
      if (rootFiles.output.data) {
        ...
        input.rootFiles = inputContent;
        if (options.resolveRemoteRefs) {
          await schemaUtils.resolveRemoteRefsMultiFile(input, options, this.computedOptions);
        }
    ...
    input.rootFiles = adaptedRootFiles;
    if (options.resolveRemoteRefs) {
      await schemaUtils.resolveRemoteRefsMultiFile(input, options);
    }
    return schemaUtils.processRelatedFiles(input, true, this.computedOptions);
  }
}
