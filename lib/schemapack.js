...
const { getConcreteSchemaUtils } = require('./common/versionUtils.js'),
  ...
  { getOptions, getSyncOptions } = require('./options'),
...
class SchemaPack {
   ...
   *
   * @param {Object} currentCollection - The existing collection to sync with
   * @param {Object} syncOptions - Options for syncing
   * @param {Function} callback - Callback function
   * @returns {void}
   */
  syncCollection(currentCollection, syncOptions, callback) {
    if (!this.validated) {
      return callback(new OpenApiErr('The schema must be validated before attempting conversion'));
    }
    ...
    try {
      ...
      return convertToOAS30IfSwagger(getConcreteSchemaUtils(this.input), this.openapi, (err, convertedOpenAPI) => {
        ...
        return v2.syncCollection(this, currentCollection, syncOptions, callback);
      });
    }
    catch (err) {
      const error = generateError(this.openapi, _.get(this.validationResult, 'specificationVersion'), err);
  ...
  static getSyncOptions(mode) {
    return getSyncOptions(mode);
  }
