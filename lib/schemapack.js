...
let path = require('path'),
  ...
  validateTransacionProcess = (transactions, schemaPackInstance, options, callback) => {
    let schema = schemaPackInstance.openapi,
      componentsAndPaths,
      analysis,
      schemaCache = {
        schemaResolutionCache: schemaPackInstance.schemaResolutionCache,
    ...
    Object.assign(componentsAndPaths, concreteUtils.getRequiredData(schemaPackInstance.openapi));
...
class SchemaPack {
    ...
    if (options.resolveRemoteRefs) {
      ...
      getRemoteReferences(remoteRefsInput, options.origin, options.remoteRefsResolver)
        .then(({ remoteRefs, specRoot }) => {
          ...
          convertSwaggerToOpenapi(concreteUtils, res.output.data.bundledContent,
            (error, newOpenapi) => {
      ...
      convertSwaggerToOpenapi(concreteUtils, this.openapi,
        (error, newOpenapi) => {
          convertProcess(error, newOpenapi, this, options, callback);
        });
    }
  ...
  validateTransaction(transactions, callback) {
    ...
    if (options.resolveRemoteRefs) {
      ...
      getRemoteReferences(remoteRefsInput, options.origin, options.remoteRefsResolver)
        .then((/* { remoteRefs, missingRemoteRefs }*/) => {
          // bundle
          validateTransacionProcess(transactions, this, options, callback);
        });
    }
    else {
      validateTransacionProcess(transactions, this, options, callback);
