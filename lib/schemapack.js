...
class SchemaPack {
  ...
  /**
   *
   * @description Extracts endpoints and all information reagrding it from OpenAPI specification
   *
   * @param {*} callback return
   * @returns {Array} - List of all endpoints
   */
  getSpecEndpoints (callback) {
    parse.parseSpecRefs(this.openapi, (err, oasSpec) => {
      let specEndpoints = [],
        globalServerUrl;
      ...
      globalServerUrl = _.get(oasSpec, 'servers[0].url', '');

      _.forEach(_.get(oasSpec, 'paths'), (pathObject, path) => {
        let pathServerUrl = _.get(pathObject, 'servers[0].url', globalServerUrl);
        _.forEach(pathObject, (operationObject, method) => {
          let specEndpoint,
            url,
            operationServerUrl;

          if (!_.includes(METHODS, method) || !_.isObject(operationObject)) {
            return;
          }

          operationServerUrl = _.get(operationObject, 'servers[0].url', pathServerUrl);
          try {
            let tempUrl = require('url').format(new URL(path, operationServerUrl),
          ...
          specEndpoint = {
            ...
            url,
            path,
            shortDescription: operationObject.summary,
            longDescription: operationObject.description,
            pathVariables: schemaUtils.parseEndpointParams(operationObject.parameters, 'path'),
            queryParameters: schemaUtils.parseEndpointParams(operationObject.parameters, 'query'),
            headers: schemaUtils.parseEndpointParams(operationObject.parameters, 'header')
          };

          if (_.isObject(operationObject.requestBody)) {
            let allMediaTypes = _.map(operationObject.requestBody.content, (contentObj, mediaType) => {
              return _.assign({ mediaType }, _.pick(contentObj, ['schema', 'example', 'examples']));
            });
            specEndpoint.body = _.pick(operationObject.requestBody, ['description', 'required']);
            !_.isEmpty(allMediaTypes) && (specEndpoint.body.mediaTypes = allMediaTypes);
          }

          if (_.isObject(operationObject.responses)) {
            let allResponses = [];
            _.forEach(operationObject.responses, (responseObj, responseCode) => {
              if (_.isObject(responseObj)) {
                let headers = _.map(responseObj.headers, (headerObj, headerName) => {
                    return _.assign({ name: headerName, in: 'header' }, headerObj);
                  }),
                  response = {
                    code: _.isInteger(_.toNumber(responseCode)) ? _.toNumber(responseCode) : 500,
                    description: responseObj.description,
                    headers: schemaUtils.parseEndpointParams(headers, 'header')
                  },
                  body = _.map(responseObj.content, (contentObj, mediaType) => {
                    return _.assign({ mediaType }, _.pick(contentObj, ['schema', 'example', 'examples']));
                  });

                !_.isEmpty(body) && (response.body = body);
                allResponses.push(response);
              }
            });
            specEndpoint.responses = allResponses;
          }
          specEndpoints.push(specEndpoint);
        });
      });
      return callback(null, specEndpoints);
    });
  }
