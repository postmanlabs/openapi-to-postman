...
module.exports = {
  ...
  */
  ...
  generateFolderObjectFromTags: function(spec) {
    ...
    // adding globalTags in the folder object that are defined at the root level
      ...
      for (let operation in currentPathObject) {
        ...
        if (!localTags || !localTags.length) {
          ...
          _.forEach(localTags, (localTag) => {
            ...
            if (!folderObj.tags.hasOwnProperty(localTag)) {
              folderObj.tags[localTag] = {
                description: '',
                requests: []
              };
            }
            // adding requests to the tag object
            folderObj.tags[localTag].requests.push({
              name: summary,
              method: operation,
              path: path,
              properties: operationObject,
