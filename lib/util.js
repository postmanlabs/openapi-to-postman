...
module.exports = {
  ...
  parseSpec: function (openApiSpec){
    ...
    rootValidation = parse.validateRoot(openApiSpec);
  ...
  getQueryParamsWithStyle: function(param, paramType){
    

  },

  // takes in a string url or a server item object
  convertPathVariables: function(pathVarArray, pathVariables){
    var variables = pathVarArray;
    _.forOwn(pathVariables, (variable) => {
        variables.push({
          key: variable.name,
          value: schemaFaker(variable.schema || {}),
          description: variable.enum ? variable.description + ' can be only one of ' + variable.enum.toString() : variable.description,
        });
    });
  ...
  addQueryParameters: function(requestObj, queryParameters){
    ...
    _.forEach(queryParameters, (param) => {
        ...
        if(param.hasOwnProperty('schema')){
          ...
          // since schema was not a required property, using the max info I can from the spec
          modifiedUrl += param.name + '=';
        }
    ...
    modifiedRequestObj.url = modifiedUrl;
    
    return modifiedRequestObj;
  },

  addHeaders: function(postmanItemObj, headers, openapi){
  ...
  convertRequestToItem: function(openapi, operationItem){
    ...
    // handling the path templating in the base url
    baseUrl = baseUrl.replace(/{/g, ':').replace(/}/g, '');

    if(baseUrl){
      pathVarArray = this.convertPathVariables([], pathVariables);
    }
    ...
    item.request.url.variable = this.convertPathVariables(pathVarArray, reqParams.path);
  
    item.request = this.addQueryParameters(item.request, reqParams.query); 

    item = this.addHeaders(item, reqParams.header, openapi); 
