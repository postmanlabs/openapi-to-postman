    ...
    if (type === 'root') {
      _.forOwn(pathVariables, (value, key) => {
        variables.push({
          key: key,
          value: '{{' + key + '}}',
          description: value.enum ? value.description ? value.description : '' +
           ' can be only one of ' + value.enum.toString() : value.description
        });
      });
    }
    else if (type === 'method') {
      _.forOwn(pathVariables, (value, key) => {
        variables.push({
          key: key,
          value: value.default,
          description: value.enum ? value.description +
           ' can be only one of ' + value.enum.toString() : value.description
        });
      });
    }
