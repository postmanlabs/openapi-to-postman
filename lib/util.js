...
*/
function safeSchemaFaker(oldSchema, bodyTypeOption, components, schemaFormat, indentCharacter) {
  var prop,
...
module.exports = {
  ...
  */
  ...
  generateFolderObjectFromTags: function(spec) {
    ...
    // checking if tags are defined at the root level
    if (globalTags.length > 0) {
      for (let tag in globalTags) {
        if (globalTags.hasOwnProperty(tag)) {
          globalTag = globalTags[tag];
          name = globalTag.name;
          if (globalTag.hasOwnProperty('description')) {
            description = globalTag.description;
          }
          // adding an object in folder object with tag name as key
          folderObj.withTags[name] = {
    ...
    for (let path in paths) {
      if (paths.hasOwnProperty(path)) {
        currentPathObject = paths[path];
        for (let operation in currentPathObject) {
          if (currentPathObject.hasOwnProperty(operation)) {
            operationObject = currentPathObject[operation];
            localTags = operationObject.tags;

            if (currentPathObject.hasOwnProperty('parameters')) {
              commonParams = currentPathObject.parameters;
            }
            operationObject.parameters = this.getRequestParams(operationObject.parameters, commonParams);
            operationObject.security = operationObject.security || spec.security;
            summary = operationObject.summary || operationObject.description;
            if (currentPathObject.hasOwnProperty('servers')) {
              pathLevelServers = currentPathObject.servers;
              collectionVariables[path + 'Url'] = pathLevelServers[0];
              delete currentPathObject.servers;
            }
            // add the request in the withoutTags object
            if (!localTags || !localTags.length) {
              folderObj.withoutTags.requests.push({
                name: summary,
                method: operation,
                path: path,
              ...
              // checking if this local tag is defined at the root level or not
                if (!folderObj.withTags.hasOwnProperty(localTag)) {
                  folderObj.withTags[localTag] = {
                    description: '',
                    requests: []
                  };
                }
                // adding requests to the tag object
                folderObj.withTags[localTag].requests.push({
                  name: summary,
                  method: operation,
                  path: path,
                  properties: operationObject,
  ...
  /**
   *
   * @param {Object} openapi object with root-level data like pathVariables baseurl
   * @param {String} tagName name of the folder to be created based on the tag name
   * @param {Object} tagObject the tag object containing the description and requests in the folder to be created
   * @returns {Object} Postman itemGroup
   */
  convertTagToItemGroup: function(openapi, tagName, tagObject) {
    var itemGroup,
      description = tagObject.description,
      requests = tagObject.requests;
    itemGroup = new sdk.ItemGroup({
      name: tagName,
      description: description
    });
    requests.forEach((request) => {
      itemGroup.items.add(this.convertRequestToItem(openapi, request));
    });
    return itemGroup;
  },
  ...
  convertToPmResponseBody: function(contentObj) {
    ...
    responseBody = this.convertToPmBodyData(contentObj[cTypeHeader], cTypeHeader,
      BODY_TYPE.RESPONSE, this.options.indentCharacter);
  ...
  convertToPmBodyData: function(bodyObj, contentType, bodyTypeOption, indentCharacter = '') {
    var bodyData = '',
    ...
    if (bodyObj.example) {
      ...
      if (this.options.schemaFaker) {
        ...
        bodyData = safeSchemaFaker(bodyObj.schema || {}, bodyTypeOption, this.components, schemaType, indentCharacter);
      }
      else {
  ...
  convertToPmBody: function(requestBody) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.convertToPmBodyData(contentObj[URLENCODED], URLENCODED, BODY_TYPE.REQUEST);
      ...
      rDataMode = 'formdata';
      bodyData = this.convertToPmBodyData(contentObj[FORM_DATA], FORM_DATA, BODY_TYPE.REQUEST);
