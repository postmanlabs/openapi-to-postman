    ...
    for (path in paths) {
      if (paths.hasOwnProperty(path)) {
        ...
        for (i = 0; i < pathLength; i++) {
          ...
          // we are maintaining children count at every folder level
          if (currentNode.name === '/') {
            currentNode.childrenCount = pathLength;
          }
          else {
            currentNode.childrenCount = (pathLength - 1) - currentPath.indexOf(currentNode.name);
          }
  ...
  convertChildToItemGroup: function(openapi, child) {
    ...
    if (resource.type === 'item-group') {
      ...
      if (resource.requestCount > 1) {
        ...
        // with parent folder.
        /* eslint-disable */
        if (resource.childrenCount === 1) {
          for (let child in resource.children) {
            itemGroup.name = resource.name + "/" + child
            if (resource.children.hasOwnProperty(child)) {
              for (let i = 0; i < resource.children[child].requests.length; i++) {
                itemGroup.items.add(
                  this.convertChildToItemGroup(openapi, resource.children[child].requests[i])
                );
              }
            }
          }
