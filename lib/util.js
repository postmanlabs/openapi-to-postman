...
* @param {string} bodyTypeOption Specifies whether the schema being faked is from a request or response.
...
*/
function safeSchemaFaker(oldSchema, resolveTo, bodyTypeOption, components, schemaFormat, indentCharacter) {
  var prop,
...
module.exports = {
  ...
  */
   ...
   * @param {*} requestType - Tells if this request body is of example request or root request
  ...
  convertToPmBodyData: function(bodyObj, requestType, contentType, bodyTypeOption, indentCharacter = '') {
    var bodyData = '',
      schemaType = SCHEMA_FORMATS.DEFAULT,
      resolveTo = '';

    if (bodyObj.example) {
      // use example values when the option is set to example
      if (this.options.requestParametersResolution === 'example' ||
      this.options.exampleParametersResolution === 'example' ||
      // if schema is not present in the bodyObject then we have to fallback to example
       !bodyObj.schema) {
      ...
      // use examples values when the option is set to example
      if (this.options.requestParametersResolution === 'example' ||
      this.options.exampleParametersResolution === 'example' ||
      // if schema is not present in the bodyObject then we have to fallback to example
       !bodyObj.schema) {
      ...
      if (this.options.schemaFaker) {
        ...
        if (requestType === REQUEST_TYPE.ROOT) {
          ...
          resolveTo = this.options.exampleParametersResolution;
        }
        bodyData = safeSchemaFaker(bodyObj.schema || {}, resolveTo, bodyTypeOption,
          this.components, schemaType, indentCharacter);
      }
      else {
  ...
  convertToPmQueryParameters: function(param, requestType) {
    ...
    if (param.hasOwnProperty('schema')) {
      ...
      paramValue = this.options.schemaFaker ?
        safeSchemaFaker(param.schema, resolveTo, BODY_TYPE.REQUEST, this.components, SCHEMA_FORMATS.DEFAULT) : '';
  ...
  convertToPmBody: function(requestBody, requestType) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.convertToPmBodyData(contentObj[URLENCODED], requestType, URLENCODED, BODY_TYPE.REQUEST);
      ...
      rDataMode = 'formdata';
      bodyData = this.convertToPmBodyData(contentObj[FORM_DATA], requestType, FORM_DATA, BODY_TYPE.REQUEST);
  ...
  convertRequestToItem: function(openapi, operationItem) {
    ...
    // TODO: Figure out what happens if type!=api-key
      ...
      if (authHelper.properties.in === 'header') {
        item.request.addHeader(this.convertToPmHeader(authHelper.properties, REQUEST_TYPE.ROOT));
        item.request.auth = {
          type: 'noauth'
        };
      }
    ...
    _.forEach(reqParams.header, (header) => {
      item.request.addHeader(this.convertToPmHeader(header, REQUEST_TYPE.ROOT));
    });
