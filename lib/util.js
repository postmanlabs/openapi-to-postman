...
module.exports = {
  ...
  */
  ...
  validateContentTypeHeader: function(cTypeHeader) {
  ...
  convertToPmResponseBody: function(contentObj) {
    ...
    for (let cType in contentObj) {
      if (this.validateContentTypeHeader(cType)) {
        cTypeHeader = cType;
    ...
    if (this.validateContentTypeHeader(cTypeHeader) && cTypeHeader.endsWith('json')) {
  ...
  convertToPmBodyData: function(bodyObj, contentType, indentCharacter = '') {
    ...
    if (bodyObj.example) {
      ...
      if (this.options.schemaFaker) {
        if (this.validateContentTypeHeader(contentType) && contentType.endsWith('xml')) {
  ...
  convertToPmResponse: function(response, code, originalRequest) {
    ...
    if (responseBodyWrapper.contentTypeHeader) {
      ...
      if (this.validateContentTypeHeader(responseBodyWrapper.contentTypeHeader) &&
      responseBodyWrapper.contentTypeHeader.endsWith('json')) {
        previewLanguage = 'json';
      }
      else if (this.validateContentTypeHeader(responseBodyWrapper.contentTypeHeader) &&
