...
function safeSchemaFaker(oldSchema, components, schemaFormat, indentCharacter, bodyType) {
  ...
  if (localSchema.properties) {
    ...
    for (prop in localSchema.properties) {
      if (localSchema.properties.hasOwnProperty(prop)) {
        if (localSchema.properties[prop].format === 'binary') {
          delete localSchema.properties[prop].format;
        }
  ...
  try {
    if (schemaFormat === SCHEMA_FORMATS.XML) {
      return xmlFaker(null, localSchema, indentCharacter);
    }
    // for JSON, the indentCharacter will be applied in the JSON.stringify step later on
    return schemaFaker(localSchema);
  }
  catch (e) {
    console.warn(
      'Error faking a schema. Not faking this schema. Schema:', localSchema,
      'Error', e
    );
    return null;
  }
...
module.exports = {
  ...
  */
  ...
  convertToPmResponseBody: function(contentObj) {
    ...
    responseBody = this.convertToPmBodyData(contentObj[cTypeHeader], cTypeHeader, BODY_TYPE.RESPONSE, this.options.indentCharacter);
  ...
  convertToPmBodyData: function(bodyObj, contentType, bodyType, indentCharacter = '') {
    var bodyData = '',
