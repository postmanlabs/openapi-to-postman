...
* @param {*} stackSize Specifies stack size to which level we fake the schema
...
*/
function safeSchemaFaker(oldSchema, stackSize, resolveTo, parameterSourceOption, components,
  schemaFormat, indentCharacter) {
  var prop,
    schema = deref.resolveRefs(oldSchema, stackSize, parameterSourceOption, components);
...
module.exports = {
  ...
  */
      ...
      _.forEach(commonPathVars, (variable) => {
        variables.push({
          ...
          value: this.options.schemaFaker ?
            safeSchemaFaker(variable.schema || {}, this.getSchemaStackSize(this.options.openApiSpecSize),
              REQUEST_TYPE.ROOT, PARAMETER_SOURCE.REQUEST, this.components, SCHEMA_FORMATS.DEFAULT) : '',
  ...
  convertChildToItemGroup: function(openapi, child) {
    ...
    if (resource.requestCount > 1) {
      ...
      if (resource.childCount === 1 && this.options.collapseFolders) {
      ...
      for (i = 0, requestCount = resource.requests.length; i < requestCount; i++) {
        itemGroup.items.add(
          this.convertRequestToItem(openapi, resource.requests[i])
        );
      }
    ...
    if (resource.requests.length === 1) {
      return this.convertRequestToItem(openapi, resource.requests[0]);
    }
  ...
  convertToPmResponseBody: function(contentObj) {
    ...
    responseBody = this.convertToPmBodyData(contentObj[cTypeHeader], REQUEST_TYPE.EXAMPLE, cTypeHeader,
      PARAMETER_SOURCE.RESPONSE, this.options.indentCharacter);
  ...
  convertToPmBodyData: function(bodyObj, requestType, contentType, parameterSourceOption, indentCharacter = '') {
    var bodyData = '',
      schemaType = SCHEMA_FORMATS.DEFAULT,
      resolveTo = this.resolveToExampleOrSchema(requestType, this.options.requestParametersResolution,
        this.options.exampleParametersResolution);
    ...
    if (bodyObj.example && (resolveTo === 'example' || !bodyObj.schema)) {
      ...
      if (this.options.schemaFaker) {
        ...
        bodyData = safeSchemaFaker(bodyObj.schema || {}, this.getSchemaStackSize(this.options.openApiSpecSize),
          resolveTo, parameterSourceOption, this.components, schemaType, indentCharacter);
      }
      else {
  ...
  convertToPmQueryParameters: function(param, requestType) {
    ...
    if (param.hasOwnProperty('schema')) {
      ...
      paramValue = this.options.schemaFaker ?
        safeSchemaFaker(param.schema, this.getSchemaStackSize(this.options.openApiSpecSize),
          resolveTo, PARAMETER_SOURCE.REQUEST, this.components, SCHEMA_FORMATS.DEFAULT) : '';
   ...
   */
  convertToPmHeader: function(header, requestType, parameterSource) {
    var fakeData,
      reqHeader,
      resolveTo = this.resolveToExampleOrSchema(requestType, this.options.requestParametersResolution,
        this.options.exampleParametersResolution);
    ...
    if (header.hasOwnProperty('schema')) {
      if (!this.options.schemaFaker) {
        ...
        fakeData = safeSchemaFaker(header.schema || {}, this.getSchemaStackSize(this.openApiSpecSize),
          resolveTo, parameterSource, this.components, SCHEMA_FORMATS.DEFAULT);
   ...
   */
  convertToPmBody: function(requestBody, requestType) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      bodyData = this.convertToPmBodyData(contentObj[URLENCODED], requestType, URLENCODED, PARAMETER_SOURCE.REQUEST);
      ...
      rDataMode = 'formdata';
      bodyData = this.convertToPmBodyData(contentObj[FORM_DATA], requestType, FORM_DATA, PARAMETER_SOURCE.REQUEST);
          ...
          _.forOwn(encoding[key].headers, (value, key) => {
            if (key !== 'Content-Type') {
              ...
              formHeaders.push(this.convertToPmHeader(encoding[key].headers[key],
                REQUEST_TYPE.ROOT, PARAMETER_SOURCE.REQUEST));
            }
          });
        }
        if (typeof value === 'object') { value = JSON.stringify(value); }
      ...
      bodyData = this.convertToPmBodyData(contentObj[bodyType], requestType, bodyType,
        PARAMETER_SOURCE.REQUEST, this.options.indentCharacter);
  ...
  convertToPmResponse: function(response, code, originalRequest) {
    ...
    _.forOwn(response.headers, (value, key) => {
      if (key !== 'Content-Type') {
        ...
        header.name = key;
        responseHeaders.push(this.convertToPmHeader(header, REQUEST_TYPE.EXAMPLE, PARAMETER_SOURCE.RESPONSE));
      }
    });
  ...
  convertRequestToItem: function(openapi, operationItem) {
    ...
    // TODO: Figure out what happens if type!=api-key
      ...
      if (authHelper.properties.in === 'header') {
        ...
        this.convertToPmQueryParameters(authHelper.properties, REQUEST_TYPE.ROOT).forEach((pmParam) => {
    ...
    _.forEach(reqParams.query, (queryParam) => {
      this.convertToPmQueryParameters(queryParam, REQUEST_TYPE.ROOT).forEach((pmParam) => {
        item.request.url.addQueryParams(pmParam);
      });
    });
    ...
    if (reqBody) {
      ...
      pmBody = this.convertToPmBody(reqBody, REQUEST_TYPE.ROOT);
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        thisOriginalRequest.url += this.convertToPmQueryArray(reqParams, REQUEST_TYPE.EXAMPLE).join('&');
        // setting headers
        _.forEach(reqParams.header, (header) => {
          originalRequestHeaders.push(this.convertToPmHeader(header, REQUEST_TYPE.EXAMPLE, PARAMETER_SOURCE.REQUEST));
        });
        thisOriginalRequest.header = originalRequestHeaders;
        // setting request body
        try {
          exampleRequestBody = this.convertToPmBody(operationItem.properties.requestBody, REQUEST_TYPE.EXAMPLE);
  ...
  */
  convertToPmQueryArray: function(reqParams, requestType) {
    let requestQueryParams = [];
    _.forEach(reqParams.query, (queryParam) => {
      this.convertToPmQueryParameters(queryParam, requestType).forEach((pmParam) => {
  ...
  /**
   * function to return schema stack size based on the openApi file size
  * @param {*} openApiSpecSize openapi spec file size in Mbs
  * @returns {*} schema stack size
  */
  getSchemaStackSize: function(openApiSpecSize) {
    // we have to restrict the memory usage to 350MBs max
    if (openApiSpecSize >= 10) {
      return 4;
    }
    else if (openApiSpecSize > 5) {
      return 6;
    }
    return 10;
  }
