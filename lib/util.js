...
module.exports = {
  ...
  */
  setPathVariablesInUrl: function (url) {
    return url.replace(/\/\{([a-zA-Z0-9\-\_]+)\}(\/|$)/, '/:$1$2');
  },
   ...
   * @param {array} operationParam opearation (postman request) level params.
   * @param {array} pathParam are path (common-substring - can be construed as a folder) level params.
  ...
  */
  convertRequestToItem: function(openapi, operationItem) {
    ...
    reqName = this.options.requestName === 'fallback' ?
      operation.summary || operation.description || operation.operationId || (displayUrl || baseUrl) : (
