...
/**
  * Verifies if an string is a url
  * @param {string} inputString - parent file name of the current node
  * @returns {boolean} - Wheter the string is a url
  */
function stringIsURL(inputString) {
  try {
    ...
    try {
      var url = require('url');
      let urlObj = url.parse(inputString);
...
function calculatePath(parentFileName, referencePath) {
  if (path.isAbsolute(referencePath) || stringIsURL(referencePath)) {
        ...
        .find(
          (key) => {
            const isLocalOutOfRoot = isOutOfRoot && isLocalRef(property, key),
              isExternal = isExtOrRemoteRef(property, key),
