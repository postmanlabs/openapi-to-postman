...
async function getReferences (currentNode, isOutOfRoot, pathSolver, parentFilename, version, rootMainKeys,
  ...
  traverseUtility(currentNode).forEach(function (property) {
    /**
     * NOTE: Compute performance diff before and after single and double traversal
     */
    ...
    if (property) {
      ...
      if (hasReferenceTypeKey) {
        ...
        const isRefEncountered = remoteRefSet.has(tempRef);
        ...
        remoteRefResolutionPromises.push(
          new Promise(async (resolveInner) => {
            const contentFromRemote = await remoteRefResolver(tempRef);
            remoteRefContentMap.set(tempRef, contentFromRemote);

            const nodeTemp = {
              fileName: tempRef,
              path: tempRef,
              content: JSON.stringify(contentFromRemote)
            };
  ...
  traverseUtility(currentNode).forEach(function (property) {
    /**
     * NOTE: Compute diff before and after single and double traversal
     */
    ...
    if (property) {
      ...
      hasReferenceTypeKey = Object.keys(property)
        .find(
          (key) => {
            const isLocalOutOfRoot = isOutOfRoot && isLocalRef(property, key),
              isExternal = isExtURLRef(property, key),
              isReferenciable = isLocalOutOfRoot || isExternal;
            return isReferenciable;
          }
        );
      ...
      if (hasReferenceTypeKey) {
        ...
        // make call
        // add to allData
        const contentFromRemote = remoteRefContentMap.get(tempRef);
        const nodeTemp = {
          fileName: tempRef,
          path: tempRef,
      ...
      if (property) {
        ...
        if (hasReferenceTypeKey) {
          let splitPathByHttp = property.$ref.split(httpSeparator),
            prefix = splitPathByHttp.slice(0, splitPathByHttp.length - 1).join(httpSeparator) + 
              httpSeparator + splitPathByHttp[splitPathByHttp.length - 1].split(localPointer)[0],
            tempRef = property.$ref,
          ...
          if (isMissingNode) {
            ...
            [nodeRef, local] = [prefix, splitPathByHttp[splitPathByHttp.length - 1].split(localPointer)[1]],
