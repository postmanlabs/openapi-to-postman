...
let path = require('path'),
  ...
  { isSwagger, isOAS31 } = require('./common/versionUtils');
...
/**
 * Returns the keys of the reusable components in the Spec
 * according to the version
 * @param {string} version - The current version
 * @returns {array} string array with the keys
 */
function getReusableComponentsKeysAccordingToVersion(version) {
  return isSwagger(version) ? COMPONENTS_KEYS_20 :
    isOAS31(version) ? COMPONENTS_KEYS_31 : COMPONENTS_KEYS_30;
}
...
function generateComponentsObject (documentContext, rootContent, refTypeResolver, components, version) {
  ...
  const COMPONENTS_KEYS = getReusableComponentsKeysAccordingToVersion(version);
